...
class TestGenericFile(unittest.TestCase):
    ...
    def test_basic_save(self):
        ...
        initial_gf = self.utils.files_coll.find_one(
            {'directory_id': self.utils.root_id, 'filename': self.utils.file.filename}, {'uploadDate': False})
    ...
    def test_rename_to(self):
        ...
        old_file = self.utils.files_coll.find_one(
            {'directory_id': self.utils.file.directory_id, 'filename': initial_filename})
    ...
    def test_new_generic_file_file(self):
        ...
        inserted_file = self.utils.files_coll.find_one(
            {'directory_id': self.utils.file.directory_id, 'filename': self.utils.file.filename})
    ...
    def test_new_generic_file_directory(self):
        ...
        GenericFile.new_generic_file(filepath=self.utils.directory.filepath,
                                     mode=0o755, file_type=GenericFile.DIRECTORY_TYPE)
        inserted_file = self.utils.files_coll.find_one(
            {'directory_id': self.utils.directory.directory_id, 'filename': self.utils.directory.filename})
    ...
    def test_new_generic_file_symbolic_link(self):
        ...
        GenericFile.new_generic_file(filepath=self.utils.symbolic_link.filepath, mode=0o755,
                                     file_type=GenericFile.SYMBOLIC_LINK_TYPE, target=self.utils.file.filename)
        inserted_file = self.utils.files_coll.find_one(
            {'directory_id': self.utils.symbolic_link.directory_id, 'filename': self.utils.symbolic_link.filename})
